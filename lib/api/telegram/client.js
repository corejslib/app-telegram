import "#lib/result";
import { Api, extensions, TelegramClient } from "teleproto";
import telegramEvents from "teleproto/events/index.js";
...
export default class TelegramClientApi {
    ...
    // public
    async createSession ( { accountId, phoneCode, password, emailAddress, emailVerification, reCaptchaCallback } = {} ) {
        if ( !accountId ) {
        ...
        if ( /^\+?\d+$/v.test( accountId ) ) {
            options = {
                ...
                "emailAddress": typeof emailAddress === "function"
                    ? emailAddress
                    : async () => emailAddress || ( await prompt( "Enter Telegram account email address: " ) ),
                emailVerification,
                reCaptchaCallback,
            };
        }
