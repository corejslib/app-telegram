    ...
    async #precessChatJoinRequest ( ctx, req ) {
        ...
        if ( permissions?.has( "telegram/bot:read" ) ) {
            ...
            if ( res.ok ) {
                ...
                await user.sendText( l10nt( "Some error occurred. Please, try again." ) );
            }
        }

        // decline
        else {
            await ctx.group.send( "declineChatJoinRequest", {
                "user_id": userId,
            } );

            if ( user ) {
                await user.sendText( l10nt( "Your group join request has been declined." ) );
            }
        }
    }
