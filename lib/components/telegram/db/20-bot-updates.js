...
BEGIN
    ...
    FOR v_row IN SELECT telegram_bot_id, chat_id, max( update_id ) AS update_id FROM new_table GROUP BY telegram_bot_id, chat_id LOOP

        UPDATE
           telegram_bot
        SET
            telegram_last_update_id = v_row.update_id
        WHERE
            id = v_row.telegram_bot_id
            AND telegram_last_update_id < v_row.update_id;

        PERFORM pg_notify( 'telegram/telegram-bot-update/' || v_row.telegram_bot_id ||'/create', json_build_object(
