...
BEGIN
    ...
    IF OLD.api_user_id != NEW.api_user_id THEN
        SELECT jsonb_set_lax( coalesce( v_data, '{}' ), '{api_user_id}', to_jsonb( NEW.api_user_id ), TRUE, 'use_json_null' ) INTO v_data;
    END IF;

    IF OLD.subscribed != NEW.subscribed THEN
        SELECT jsonb_set_lax( coalesce( v_data, '{}' ), '{subscribed}', to_jsonb( NEW.subscribed ), TRUE, 'use_json_null' ) INTO v_data;
    END IF;

    IF OLD.returned != NEW.returned THEN
        SELECT jsonb_set_lax( coalesce( v_data, '{}' ), '{returned}', to_jsonb( NEW.returned ), TRUE, 'use_json_null' ) INTO v_data;
    END IF;
    ...
    IF v_data IS NOT NULL THEN
        SELECT jsonb_set( v_data, '{id}', to_jsonb( NEW.telegram_user_id ), TRUE ) INTO v_data;

        PERFORM pg_notify( 'telegram/telegram-bot-user/' || NEW.telegram_bot_id || '/update', v_data::text );
    END IF;
