...
export default Super =>
    class extends Super {
        ...
        async #getReportMessage ( ctx ) {
            ...
            return {
                "text": l10nt( locale => {
                    return locale.l10n( msgid`Bot's statistics (${ locale.formatDate( new Date(), "dateStyle:short,timeStyle:short" ) }):

Total users: <b>${ res.data.total_users }</b>
Subscribed users: <b>${ res.data.total_subscribed_users }</b>
Unsubscribed users: <b>${ res.data.total_unsubscribed_users }</b>
` );
                } ),
                "parse_mode": "HTML",
                "reply_markup": {
