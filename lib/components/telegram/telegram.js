import { fromMsgPack, toMsgPack } from "#core/msgpack";
...
export default class Telegram {
    ...
    createCallbackData ( method, ...args ) {
        const value = toMsgPack( [ method, ...args ], "base64url" );
    ...
    decodeCallbackData ( string ) {
        try {
            return fromMsgPack( string, "base64url" );
        }
        catch {
            return null;
        }
    ...
    async #downloadFile ( req ) {
        const [ botId, fileId ] = req.path.slice( this.config.filesUrl.length ).split( "/", 2 );
    ...
    async #downloadAvatar ( req ) {
        const [ botId, userId ] = req.path.slice( this.config.avatarUrl.length ).split( "/", 2 );
